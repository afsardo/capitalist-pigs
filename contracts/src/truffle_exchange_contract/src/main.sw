...
impl TruffleExchange for Contract {
    ...
    fn get_pool_info() -> PoolInfo {
        PoolInfo {
            ...
            truffle_reserve: get_current_reserve(get::<b256>(TOKEN_ID_KEY)),
            lp_truffle_supply: storage.lp_truffle_supply,
        }
    }

    #[storage(read)]
    fn get_position(amount: u64) -> PositionInfo {
        let total_liquidity = storage.lp_truffle_supply;
        let eth_reserve = get_current_reserve(ETH_ID);
        let truffle_reserve = get_current_reserve(get::<b256>(TOKEN_ID_KEY));
        let eth_amount = mutiply_div(amount, eth_reserve, total_liquidity);
        let truffle_amount = mutiply_div(amount, truffle_reserve, total_liquidity);

        PositionInfo {
            lp_truffle_supply: total_liquidity,
    ...
    fn get_add_liquidity(amount: u64, asset_id: b256) -> PreviewAddLiquidityInfo {
        ...
        let total_liquidity = storage.lp_truffle_supply;
        let eth_reserve = get_current_reserve(ETH_ID);
        let truffle_reserve = get_current_reserve(token_id);
        ...
        if total_liquidity > 0 {
            truffle_amount = mutiply_div(current_eth_amount, truffle_reserve, eth_reserve);
            lp_truffle_received = mutiply_div(current_eth_amount, total_liquidity, eth_reserve);
        } else {
            lp_truffle_received = current_eth_amount;
        };

        if (asset_id == token_id) {
            truffle_amount = current_eth_amount;
        }

        PreviewAddLiquidityInfo {
            truffle_amount: truffle_amount,
            lp_truffle_received: lp_truffle_received,
        }
    }
    ...
    fn add_liquidity(min_liquidity: u64, deadline: u64) -> u64 {
        ...
        let current_truffle_amount = storage.deposits.get((sender, ~ContractId::from(get::<b256>(TOKEN_ID_KEY))));
        ...
        if total_liquidity > 0 {
            ...
            let eth_reserve = get_current_reserve(ETH_ID);
            let truffle_reserve = get_current_reserve(get::<b256>(TOKEN_ID_KEY));
            let truffle_amount = mutiply_div(current_eth_amount, truffle_reserve, eth_reserve);
            ...
            if (current_truffle_amount >= truffle_amount) {
                // Add fund to the reserves
                add_reserve(get::<b256>(TOKEN_ID_KEY), truffle_amount);
                add_reserve(ETH_ID, current_eth_amount);
                // Mint LP token
                mint(liquidity_minted);
                storage.lp_truffle_supply = total_liquidity + liquidity_minted;
                ...
                let token_extra = current_truffle_amount - truffle_amount;
                if (token_extra > 0) {
                ...
                transfer_to_address(current_truffle_amount, ~ContractId::from(get::<b256>(TOKEN_ID_KEY)), sender);
            ...
            // Add fund to the reserves
            add_reserve(get::<b256>(TOKEN_ID_KEY), current_truffle_amount);
            add_reserve(ETH_ID, current_eth_amount);

            // Mint LP token
            mint(initial_liquidity);
            storage.lp_truffle_supply = initial_liquidity;
    ...
    fn remove_liquidity(min_eth: u64, min_tokens: u64, deadline: u64) -> RemoveLiquidityInfo {
        ...
        let truffle_amount = mutiply_div(msg_amount(), truffle_reserve, total_liquidity);

        assert((eth_amount >= min_eth) && (truffle_amount >= min_tokens));

        burn(msg_amount());
        storage.lp_truffle_supply = total_liquidity - msg_amount();

        // Add fund to the reserves
        remove_reserve(get::<b256>(TOKEN_ID_KEY), truffle_amount);
        remove_reserve(ETH_ID, eth_amount);

        // Send tokens back
        transfer_to_address(eth_amount, ~ContractId::from(ETH_ID), sender);
        transfer_to_address(truffle_amount, ~ContractId::from(get::<b256>(TOKEN_ID_KEY)), sender);
    ...
    fn swap_with_minimum(min: u64, deadline: u64) -> u64 {
        ...
        let mut bought = 0;
        if (asset_id == ETH_ID) {
            let tokens_bought = get_input_price(forwarded_amount, eth_reserve, truffle_reserve);
            ...
            let eth_bought = get_input_price(forwarded_amount, truffle_reserve, eth_reserve);
    ...
    fn swap_with_maximum(amount: u64, deadline: u64) -> u64 {
        ...
        let mut sold = 0;
        if (asset_id == ETH_ID) {
            let eth_sold = get_output_price(amount, eth_reserve, truffle_reserve);
            ...
            let tokens_sold = get_output_price(amount, truffle_reserve, eth_reserve);
    ...
    fn get_swap_with_minimum(amount: u64) -> PreviewInfo {
        ...
        if (msg_asset_id().into() == ETH_ID) {
            sold = get_input_price(amount, eth_reserve, truffle_reserve);
            has_liquidity = sold < truffle_reserve;
        } else {
            sold = get_input_price(amount, truffle_reserve, eth_reserve);
    ...
    fn get_swap_with_maximum(amount: u64) -> PreviewInfo {
        ...
        let mut has_liquidity = true;
        if (msg_asset_id().into() == ETH_ID) {
            assert(amount < truffle_reserve);
            sold = get_output_price(amount, eth_reserve, truffle_reserve);
            has_liquidity = sold < eth_reserve;
        } else {
            assert(amount < eth_reserve);
            sold = get_output_price(amount, truffle_reserve, eth_reserve);
            has_liquidity = sold < truffle_reserve;
        }
        PreviewInfo {
            amount: sold,
